...
/**
 * Register suggested privacy policy text for site owners.
 */
function basksc_register_privacy_policy_content()
{
    if (!function_exists('wp_add_privacy_policy_content')) {
        return;
    }

    $is_ja = (strpos((string) get_locale(), 'ja') === 0);

    if ($is_ja) {
        $content = '
<p>Basketball Scorebook プラグインは、試合中に入力したスコアブックデータをブラウザ内に保存します。このデータは通常、WordPress サイトのデータベースには保存されず、利用者が使用している端末とブラウザの LocalStorage に保存されます。</p>
<p>保存されるデータには、チーム名、選手名、背番号、得点、ファウル、クォーター時刻、メモなど、スコアブック入力に必要な情報が含まれる場合があります。これらのデータは、同じ端末・同じブラウザで再訪した際に復元されますが、ブラウザの保存データ削除や端末変更によって失われることがあります。</p>
<p>このプラグインは、匿名利用統計の送信を管理者が明示的に有効化した場合に限り、外部の Matomo 分析サーバーへ最小限の利用イベント情報を送信します。送信される情報には、プラグインのバージョン、WordPress バージョン、PHP バージョン、サイトロケール、イベント名が含まれます。サイト URL、メールアドレス、スコアブックの入力内容自体は送信されません。</p>
<p>重要な試合データを扱う場合は、必要に応じて JSON 書き出し機能などを用いて別途バックアップを取得してください。</p>';
    } else {
        $content = '
<p>The Basketball Scorebook plugin stores scorebook data entered during a game in the visitor&apos;s browser. This data is normally not stored in the WordPress database; instead, it is saved in the LocalStorage area of the device and browser being used.</p>
<p>The stored data may include information required to operate the scorebook, such as team names, player names, jersey numbers, scores, fouls, quarter times, and notes. This data may be restored when the same device and browser are used again, but it can be lost if the browser storage is cleared or the device is changed.</p>
<p>This plugin can also send minimal anonymous usage events to an external Matomo analytics server, but only when the site administrator has explicitly enabled the opt-in setting. The transmitted information includes the plugin version, WordPress version, PHP version, site locale, and event name. The site URL, email addresses, and the actual scorebook contents are not sent.</p>
<p>For important game records, consider creating a separate backup by using the JSON export feature or another backup method as needed.</p>';
    }
...
add_action('admin_init', 'basksc_register_privacy_policy_content');
...
function basksc_handle_review_prompt()
{
    ...
    if ($target === 'help') {
        ...
        $url = ($locale === 'ja')
            ...
            : 'https://doc778.com/help-scorebook-en';
        wp_safe_redirect($url);
        exit;
    }
