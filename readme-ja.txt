...
* **データ永続化とバックアップ:** ブラウザ的LocalStorageに自動保存されるため、不意のページ更新でもデータは失われません。ただし、ブラウザのキャッシュ削除や端末故障に備え、重要な試合では「JSONエクスポート」機能による外部保存を強く推奨します。
* **JSONインポート/エクスポート:** 試合データをJSONファイルとして保存し、バックアップ、共有、テンプレートとして利用可能
* **データベース肥大化なし:** すべてのデータはクライアントサイド（Alpine.js）で処理され、WordPressデータベースを汚しません

**使用方法:**

任意の固定ページまたは投稿にショートコード `[basketball_scorebook]` を追加するだけです。
最適な表示のため、「全幅（Full-Width）」ページテンプレートの使用をおすすめします。

== デモと使い方 ==

プラグインをインストールせずに、公式デモサイトでスコアブックの全機能をお試しいただけます。
...
**使い方ガイド（日本語）:** [https://doc778.com/help-scorebook](https://doc778.com/help-scorebook)
...
1. `basketball-scorebook` フォルダを `/wp-content/plugins/` ディレクトリにアップロードします。
2. WordPressの「プラグイン」メニューからプラグインを有効化します。
3. 任意のページコンテンツに `[basketball_scorebook]` ショートコードを配置します。

== よくある質問 ==

= インターネット接続がなくても動作しますか？ =

はい、ページが一度読み込まれればオフラインでも動作します。すべての計算はJavaScript（Alpine.js）を使ってブラウザ内で行われます。

= データが消えることはありますか？ =

通常の使用でデータが消えることはありません。ただし、ブラウザの閲覧履歴（キャッシュ）を削除すると、LocalStorageのデータも一緒に削除されます。確実なデータ保護のため、ハーフタイム時や試合終了後に「JSONエクスポート」を行うことをおすすめします。

= スコアブックをPDFとして保存できますか？ =

はい。「印刷/PDF」ボタンをクリックすると、印刷ダイアログが開きます。送信先で「PDFに保存」を選択してください。A4横向きに最適化されています。

= データは自動で保存されますか？ =

はい。スコアブックは変更があるたびに、ブラウザのLocalStorageへ自動的に保存されます。
...
* 更新: WordPress 7.1 までの動作確認を実施
* 改善: ミニバス版とのデータ相互互換を保つため、スキーマおよびアダプタ層を同期（選手行・コーチ欄の消し線状態 `rosterCancelled` / `coachCancelled` / `assistantCoachCancelled` のパラメータ共有に対応）

= 1.0.8 =

* 修正: 保存処理の際に、最新の試合イベントログやアノテーションが古いストレージデータで上書きされて戻ってしまう（先祖返りする）重大な不具合を修正
* 改善: 将来のミニバス版との同期および移行に備え、保存データ構造を拡張（試合時間設定 `meta.quarterLength`/`overtimeLength` および選手配置の座標 `liveState.homeCourtPositions`/`awayCourtPositions` を裏側で保持するように対応）
...
* 更新: ミニバス版移行準備として保存データスキーマを拡張（`meta` と `foulEvents` を保存/書出/読込に追加。LocalStorageキーは維持）
...
WordPress 7.1 での動作確認に対応し、ミニバス版とのパラメータ共有化によってデータ互換性を向上させました。

= 1.0.8 =

保存処理時の潜在的なデータ先祖返りバグを修正し、データの信頼性を向上させました。将来的にミニバス版とのデータ移行・同期を予定している場合は、アップデートを強く推奨します。
